...
BACKGROUNDS_PATTERN = "background/*"
...
OUT_DIR = "result/{}".format(DATE)

#Create classes array
CLASSES_PATTERN ="data/objects/*"
  ...
  """
    ...
    transform_objects = [
        ...
        flip.transformers.data_augmentation.RandomResize(
            mode='symmetric_w',
            relation='parent',
            w_percentage_min=0.03,
            w_percentage_max=0.04
        )
    ]
    ...
    pre_mask = cv2.imread(r"segmentation\0_seg0.png",cv2.IMREAD_UNCHANGED)
    kernel = np.ones((11,11),np.uint8)
    pre_mask = cv2.erode(pre_mask,kernel,iterations =7)  
    ...
    transform = flip.transformers.Compose(
        [
            ...
            flip.transformers.domain_randomization.ObjectsRandomPosition(
                x_min=0, y_min=0, x_max=1, y_max=1, mode='percentage', pre_mask=pre_mask
            ),
...
def create_google_csv(elements):
    ...
    with open("result/google_data.csv", mode="w") as f:
        f.write(csv_data)
